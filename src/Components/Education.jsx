...
const Education = () => {
  ...
  const certifications = [
    {
      title: "Pearson Certified Test Administrator",
      year: "2026",
      type: "Certification",
      issuer: "Pearson",
    },
    {
      title: "Final Year Project Showcase - Open House & Job Fair",
      year: "2026",
      type: "Event",
  ...
  return (
    <section
      id="education"
      className="py-32 bg-gradient-to-br from-gray-50 via-white to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-indigo-900"
    >
      <div className="container mx-auto px-6 max-w-6xl">
            ...
            {education.map((edu, index) => {
              ...
              return (
                <div key={edu.degree} className="relative group">
                  {/* Timeline dot */}
                  <div
                    className={`absolute left-6 w-4 h-4 bg-gradient-to-r ${edu.color} rounded-full shadow-lg ring-4 ring-white dark:ring-gray-800 group-hover:scale-125 transition-transform duration-300`}
                  ></div>

                  {/* Education Card */}
                  <div className="ml-20">
                    <div
                      className={`relative bg-gradient-to-r ${edu.bgColor} rounded-3xl p-8 shadow-lg hover:shadow-2xl transition-all duration-500 group-hover:-translate-y-2 border border-white/50 dark:border-gray-700/50 backdrop-blur-sm`}
                    >
                      {/* Card Header */}
                      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between mb-6">
                        <div className="flex items-start space-x-4">
                          <div
                            className={`flex-shrink-0 w-12 h-12 bg-gradient-to-r ${edu.color} rounded-xl flex items-center justify-center shadow-lg`}
                          >
        ...
        <div className="mt-16 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-3xl shadow-lg p-8 border border-gray-200/50 dark:border-gray-700/50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {certifications.map((item) => (
              <div
                key={item.title}
                className="p-5 rounded-2xl border border-indigo-100 dark:border-indigo-900/40 bg-gradient-to-r from-indigo-50/70 to-blue-50/70 dark:from-indigo-900/20 dark:to-blue-900/20"
              >
                ...
                <h4 className="text-lg font-semibold text-gray-900 dark:text-white leading-snug mb-2">
                  {item.title}
                </h4>

                <p className="inline-flex items-center text-sm text-gray-600 dark:text-gray-300">
                  <BadgeCheck className="w-4 h-4 mr-1.5 text-indigo-600 dark:text-indigo-400" />
                  {item.issuer}
                </p>
              </div>
            ))}
